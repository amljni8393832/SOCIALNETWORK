...
$sql = "SELECT m.id, m.object, m.text, m.date, u.nom, u.photo
        FROM message m
        JOIN users u ON m.expediteur = u.id
...
<div class="container">
    <div class="leftnav">
        ...
        <a href="nouveauxMessage.php" class="nav-active">Nouveaux messages</a>
        <a href="nouveauxEnvoi.php">Messages envoyés</a>
    </div>
    <div class="content">
        <div class="card">
                ...
                while($row = $result->fetch_assoc()){
                    ...
                    echo '</div>
                          <div class="friend-info">
                            ...
                            <p>'.$row['text'].'</p>
                            <p><em>Reçu le '.date('d/m/Y H:i', strtotime($row['date'])).'</em></p>
                          </div>
                          </div>';
                }
                echo '</div>';
            } else {
                echo '<p>Vous n\'avez pas encore reçu de messages.</p>';
            }
            ?>
        </div>
    </div>
